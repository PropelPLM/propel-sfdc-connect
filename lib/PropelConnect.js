 ...
 * REST Api.
 */
export default class PropelConnect {
  /**
  ...
  /**
   * This is same as simpleQuery but only processes this.querySize ids at a time
   * @param {string} queryString
   * @param {string[]} queryList
   */
   simpleQueryExtend(queryString, queryList) {
    return new Promise(async (resolve, reject) => {
      if (!queryList || !queryList.length) { resolve([]) }
      let result = []
      let start = 0
  ...
  uploadFile(eventId, data) {
    return new Promise((resolve, reject) => {
      const filename = `Data_Service_Logs_${eventId}.json`
      const dataLoad =
