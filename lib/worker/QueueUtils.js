...
```
const { logger, createMessageBroker, startQueueConsumer, startQueueProducer, sendToQueue } = require('@propelsoftwaresolutions/propel-sfdc-connect');
...
const start = async () => {
    ...
    try {
        await sendToQueue(producer, { foo: 'bar' });
    } catch (e) {
...
/**
 * Attempt to publishes a message to a RabbitMQ queue.
 * 
 * @param {*} producer A producer created using startQueueProducer
 * @param {*} content Content to publish
 */
export const sendToQueue = async (producer, content) => {
    // check if the channel is connected
    if (!producer.channel) {
        throw new Error('Producer channel is not connected.');
    }
    ...
    // Publish message
    const published = await producer.channel.sendToQueue(
        producer.config.queue,
        buffer,
        {
            persistent: producer.config.options?.persistent !== false, // default to true
            headers: producer.config.headers || {}
        }
    );
}
