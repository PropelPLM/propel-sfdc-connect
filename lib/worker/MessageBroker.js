...
export default class MessageBroker {
    ...
    async addProducer(producerConfig) {
        const newProducer = new QueueProducer({
    ...
    */
    ...
    async handleConsumerError(consumer, msg, error) {
        ...
        if (retryCount <= maxRetries) {
            try {
                // Publish a new copy of the message with updated retry count
                ...
                logger.error('Failed to requeue message:', error);
                // If we failed to requeue, nack without requeue to prevent loss
...
class QueueProducer {
    ...
    async send (content) {
        ...
        await this.channel.sendToQueue(
            ...
            buffer,
            {
                persistent: this.config.options?.persistent !== false, // default to true
