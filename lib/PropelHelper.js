import PropelLog from './PropelLog';

export default class PropelHelper {
  /**
  ...
  async cloneObject(objectName, revIdString, oldNewRevId) {
     ...
     * @returns SObject
     */
    findById(sObjectType, id, applyNamespace = true) {
      if (applyNamespace && this.namespaceString && !sObjectType.toLowerCase().startsWith(this.namespaceString.toLowerCase() + '__')) {
          sObjectType = this.namespaceString + '__' + sObjectType;
      }
      return this.connection.sobject(sObjectType).retrieve(id);
  }
}
