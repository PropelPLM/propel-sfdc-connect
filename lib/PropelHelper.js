...
class PropelHelper {
   ...
   */
  constructor(connection, mapping, options, namespace) {
    // static
  ...
  async cloneObject(objectName, revIdString, oldNewRevId) {
    ...
    for (let i = 0; i < insertResult.length; i++) {
      ...
      if (!res.success) {
        this.executor.logs.push(new PropelLog(`${objectName} (New Draft)`, obj.Id, obj.Name, res.success, res.errors.map((e) => '' + e), true))
      }
    }
